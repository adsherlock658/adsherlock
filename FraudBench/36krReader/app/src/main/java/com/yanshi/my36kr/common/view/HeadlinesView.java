...
public class HeadlinesView extends FrameLayout {
    ...
    void printSamples(MotionEvent ev) {
        ...
        Log.d("ViewGroup", "In printSamples historySize:"+historySize );
        Log.d("ViewGroup", "In printSamples pointerCount:"+pointerCount );
        for (int h = 0; h < historySize; h++) {
            System.out.printf("At time %d:", ev.getHistoricalEventTime(h));
            Log.d("ViewGroup", "In printSamples At time %d:" + ev.getHistoricalEventTime(h) );

            for (int p = 0; p < pointerCount; p++) {
                Log.d("ViewGroup", "pointer"+ev.getPointerId(p)+":"+ev.getHistoricalX(p, h)+","+ev.getHistoricalY(p, h));
                System.out.printf("  pointer %d: (%f,%f)",
                        ev.getPointerId(p), ev.getHistoricalX(p, h), ev.getHistoricalY(p, h));
            }
        }
        System.out.printf("At time %d:", ev.getEventTime());
        Log.d("ViewGroup", "At time %d:"+ev.getEventTime());
        for (int p = 0; p < pointerCount; p++) {
            Log.d("ViewGroup", "pointer"+ev.getPointerId(p)+":"+ev.getX(p)+","+ev.getY(p));
            System.out.printf("  pointer %d: (%f,%f)",
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        Log.d("ViewGroup", "MyLayout dispatchTouchEvent: " + ev.getAction());
        printSamples(ev);
        switch (ev.getAction()) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        Log.d("ViewGroup", "MyLayout View ontouchevent: " + event.getAction());
