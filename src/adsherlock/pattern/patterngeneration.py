...
def BayesSignature(nonads, ads):
    ...
    for t in tokenNumDict:
        ...
        tokenScoreDict['nonads'] = nonadscore
        tokenScoreDict['ads'] = adscore
...
if __name__ == "__main__":
    ...
    print "==========================tokenScoreDict===================="
    print "token, adscore, nonadscore"
    for token in tokenScoreDict:
        print token, tokenScoreDict[token]["ads"], tokenScoreDict[token]["nonads"]
    # resultTokens = cluster(nonads, nonadarff)
   # print ads
    #print XXX
    nonadclusterSet = cluster(nonads, ads)
    for indexset in nonadclusterSet:
        tokensets = []
        for i in indexset:
            tokensets.append(nonads[i])
        print "**************"
        print indexset
        nonadsignature = signature_generating(tokensets)
        adscore = 0
        nonadscore = 0
        for t in nonadsignature:
            adscore += tokenScoreDict[t]["ads"]
            nonadscore += tokenScoreDict[t]["nonads"]
        print nonadsignature, adscore, nonadscore
    print "============================ads==========================="
    adclusterSet = cluster(ads, nonads)
    for indexset in adclusterSet:
        tokensets = []
        for i in indexset:
            tokensets.append(ads[i])
        print "**************"
        print indexset
        print signature_generating(tokensets)